    ...
    if len(lines[0]) == 5:  # if there are match results
        ...
        for line in lines:
            ...
            if diff > 0:
                ...
                newlabel = 0 if diff < 6 else (1 if diff < 16 else 2)
                history[int(line[0].strip())][int(line[1].strip())] += (newlabel)**2
            else:
                # if (-4, 0] 3; if (-11, -4] 4; if (..., -11] 5
                newlabel = 3 if diff > -6 else (4 if diff > -16 else 5)
                history[int(line[0].strip())][int(line[1].strip())] -= (newlabel - 3)**2
...
def train(training_set, label_set):
    ...
    for k in range(1):
        ...
        for i in range(length):
            if (int(classes[i][0].decode('utf8')) - 2.5) * \
             (test_label_set[i] - 2.5) > 0:
                right += 1

        test_label_set = [0 if x < 2.5 else 1 for x in test_label_set]

        print('train steps number: ', (k+1)*1000)
...
if __name__ == '__main__':
    ...
    classifier, pca = train_all(training_set, label_set)
